...
- **Primary Research Tool**: `perplexity/perplexity_ask` - Use EXCLUSIVELY for all web research and external knowledge queries
- **No Direct Model Inference**: Research findings come from Perplexity MCP server only, ensuring accuracy and consistency
...
Use `#tool:runSubagent` to delegate comprehensive context research:

```
Use this subagent autonomously without pausing:
- Analyze the codebase semantically
- Examine relevant configuration files
- Query the user's infrastructure via terminal (kubectl, talosctl)
- Research external requirements via perplexity/perplexity_ask
Return findings without interruption
```

**If `runSubagent` unavailable**: Gather context yourself using:
- Terminal queries for infrastructure state
- `perplexity/perplexity_ask` for research and external knowledge
- File reading for semantic understanding
...
### perplexity/perplexity_ask (Research)
âœ… Use for:
- External knowledge (frameworks, tools, best practices)
- Current information (new releases, updates)
- Technical explanations and comparisons
- Context you don't have locally
...
- Research GPU passthrough + Talos patterns via perplexity
- Return findings
